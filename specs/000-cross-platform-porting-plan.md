...
| 대단위 | 목적 | 포함 | 상태/의존 |
|---|---|---|---|
| **A. macOS 기능 완성** 🔄 | 원본 동등 "매일 쓰는 앱" | 자막 스타일·위치 + **설정 영속**(F6) · 번역음성 재생+덕킹+게인(F11,F12) · 클라이언트 VAD(F13) · 비용 HUD(F3) · 녹화(F4) · 키체인 UI(F7) · 모델카탈로그(F5) · macOS 무설치 탭(F9, 선택) · HUD/트레이 폴리시 → [012](012-a-macos-feature-completion.md) | mac 완결. 진행 중 |
