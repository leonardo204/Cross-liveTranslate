...
| Phase | 목표 | 산출물 | 검증 |
|---|---|---|---|
| **P0** ✅ | 스캐폴드 | Wails 앱 부팅, 버전 주입, updater 이식, CI, 빌드 검증 | mac/win 빌드 통과 |
| **P1** ✅ | 코어 번역(headless) | Gemini WS 클라이언트(F1,F19) + 오디오 캡처(F8) + 마이크 입력 → 콘솔 자막 | **실키·실마이크 한→영 번역 검증 완료** |
| **P2** 🔄 | 루프백 + 파이프라인 | 시스템 루프백(F9,F10) + reconciler(epoch) + 자막엔진(F2) + 이벤트모델 → [010](010-p2-loopback-reconciler-subtitle.md) (P2a 코어 / P2b macOS 탭) | 시스템 소리 번역 |
| **P3** 🔄 | UI | 오버레이 창(F14) + 제어 HUD(F15) + 설정창(F16) + 트레이(F17) + 멀티모니터(F18) → [011](011-p3-overlay-ui-architecture.md). **P3a ✅**(투명 클릭통과 오버레이) **P3b ✅**(controller+IPC+트레이+파이프라인, E2E 실키 검증) / P3c 자막스타일 남음 | **실행→발화→화면 자막 동작 확인** |
P0~P3b로 **macOS에서 앱이 E2E 동작**(실행→발화→화면 자막). 이후 남은 작업은 아래 **3개 대단위**로 재분류한다(세부 phase 통합).

### 남은 대단위 (재분류)

| 대단위 | 목적 | 포함 | 상태/의존 |
|---|---|---|---|
| **A. macOS 기능 완성** ✅ | 원본 동등 "매일 쓰는 앱" | ✅설정 영속+키체인(F6,F7) · ✅자막 스타일·위치 · ✅번역음성 재생+덕킹+게인(F11,F12) · ✅비용 HUD(F3) · ✅녹화(F4) · ✅에너지 VAD(F13) → [012](012-a-macos-feature-completion.md). *잔여(선택): macOS 무설치 탭(F9, BlackHole로 대체 동작), Silero ONNX 업그레이드, HUD/트레이 폴리시* | **mac 기능 완성 — 실키 검증** |
| **B. Windows 실전화** | 진짜 크로스플랫폼 완성 | 오버레이 layered/클릭통과 · 트레이(systray) · WASAPI 루프백 실동작 · cgo mingw/러너 빌드 · 자식 spawn+IPC 실동작 | Windows 머신/러너 필요. 코드 대부분 완비 |
| **C. 배포 파이프라인** | 배포 + 자동업데이트 실전 | 서명·노터라이즈 DMG + Windows exe · minisign 자동업데이트 E2E(vN→vN+1) · GitHub Actions 매트릭스 + `latest.json` 병합 | 스크립트·기계장치 완비(P0). 기능 안정 후 |

**권장 순서: A → C(mac 우선 배포) → B(Windows).** 각 대단위 종료 시 `verify`로 실제 동작 확인(빌드 통과만으로 완료 선언 금지).
